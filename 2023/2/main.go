...
func main() {
	input := `Game 1: 5 red, 1 green; 6 red, 3 blue; 9 red; 1 blue, 1 green, 4 red; 1 green, 2 blue; 2 blue, 1 red
...
Game 100: 10 blue, 5 green; 4 green, 3 red, 6 blue; 2 green, 4 red, 1 blue`
	...
	for _,l := range lines {
		...
		gameID := tonum(split[0][5:])
		split = strings.Split(split[1], ";")
		for _,r := range split {
			for _,c := range strings.Split(r, ",") {
				p := strings.Split(strings.TrimSpace(c), " ")
				num := tonum(p[0])
				col := p[1]
				if num > maxes[col] { 
					maxes[col] = num
		...
		fmt.Println(gameID, maxes, power)
		sum += power
	}
	fmt.Println(sum)
}
